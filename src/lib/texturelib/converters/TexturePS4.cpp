...
TexturePS4::TexturePS4()
: ToolTexture( 0 )
{
	mMetaData.mPlatform = PLATFORM_PS4;
}

TexturePS4::TexturePS4( uint32_t num_mips )
...
TexturePS4* TexturePS4::Convert( TextureType::Type type, PixelFormat::Type pixel_format, uint32_t conversion_flags, InputImages& input_images )
{
	const uint32_t num_levels = NumMipLevels( pixel_format, input_images );
	TexturePS4* texture = new TexturePS4( num_levels );
	...
	for( uint32_t i = 0; i < num_levels; ++i )
	{
		...
		const void* converted_pixel_data = PixelFormatConverter::Convert( pixel_format, image, converted_size, PLATFORM_PS4 );
...
void TexturePS4::InternalSerialize( StreamWriter& writer ) const
{
...
bool TexturePS4::DeserializeData( BinaryBufferReader* reader )
{
	for( uint32_t i = 0; i < NumMips(); ++i )
	{
...
void* TexturePS4::ToARGB( uint32_t mip_level ) const
{
